...
tags = {
  ...
  layout = { layouts[2], layouts[2], layouts[2], layouts[1], layouts[2],layouts[2],layouts[17],layouts[17],layouts[2], layouts[2], 
}}
for s = 1, screen.count() do
...
screenshots = {
   { "Now", "scrot '%Y-%m-%d--%s_$wx$h_scrot.png' -e 'mv $f /home/alex/Pictures/Screenshots/ &amp; sxiv /home/alex/Pictures/Screenshots/$f'" },
   { "Wait 5s", "scrot -d 5 '%Y-%m-%d--%s_$wx$h_scrot.png' -e 'mv $f /home/alex/Pictures/Screenshots/ &amp; sxiv /home/alex/Pictures/Screenshots/$f'" },
   { "Wait 10s", "scrot -d 10 '%Y-%m-%d--%s_$wx$h_scrot.png' -e 'mv $f /home/alex/Pictures/Screenshots/ &amp; sxiv /home/alex/Pictures/Screenshots/$f'" },
   { "Select", "scrot -s '%Y-%m-%d--%s_$wx$h_scrot.png' -e 'mv $f /home/alex/Pictures/Screenshots/ &amp; sxiv /home/alex/Pictures/Screenshots/$f'" }
}

table.insert(menu_items, { "screenshots", screenshots })
...
--cpu mouse -- LeftClick=1 RightClick=3 ScrollUp=4 ScrollDown=5
...
vicious.register(mem, vicious.widgets.mem, '<span color="' .. blue .. '">RAM:</span> $1%', 5)

--mem mouse -- LeftClick=1 RightClick=3 ScrollUp=4 ScrollDown=5
...
-- Mail IMAP check [doesn't seem to work properly]
...
mpdwidget:buttons(awful.util.table.join(
	awful.button({ }, 3,function() awful.util.spawn(ncmpcpp, true) end),
	awful.button({ }, 1,function () awful.util.spawn_with_shell("mpc toggle || ncmpcpp toggle || ncmpc toggle || pms toggle")
    end)))
...
netupinfo = lain.widgets.net({
    settings = function()
        widget:set_markup(markup(blue, " / ") .. net_now.sent)
        netdowninfo:set_markup(markup(blue, "Net: ") .. net_now.received)
...
--redshift:attach(myredshift,   function ()
--                            if redshift:is_active() then
--                               text_is_active = markup(grey, redshift:is_active_text())
--                            else
--                               text_is_active = markup(grey, redshift:is_active_text())
--                            end
--                            myredshift:set_markup(markup(blue, "RS: ") .. text_is_active)
--                          end )
...
-- screenshots
    awful.key({ }, "Print", function () awful.util.spawn_with_shell("scrot -e 'mv $f /home/alex/Pictures/Screenshots/'") end),
    awful.key({ modkey, "Shift" }, "Print", function () awful.util.spawn_with_shell("scrot -s") end),
--awful.key({ modkey, "Shift" }, "Print", function () awful.util.spawn_with_shell("scrot -s -e 'mv $f /home/alex/Pictures/Screenshots/'") end),
-- Toggle redshift
awful.key({ modkey, "Control" }, "y", function () redshift:toggle() end),
    --lain.widgets.contrib.redshift:toggle({modkey}, "y", redshift.toggle),
-- Useless gaps
...
--    awful.key({ modkey, "Shift"   }, "q", awesome.quit), -- Default M-S-q command, oblogout lets you choose what to do
    ...
    awful.key({ modkey, "Shift"   }, "q", function () awful.util.spawn_with_shell("sudo hibernate") end),
    awful.key({ modkey, "Control"   }, "q", function () awful.util.spawn_with_shell("sudo hibernate-ram") end),
...
--        local cmenu = awful.menu.clients({width=245}, {keygrabber=true, coords={x=600, y=330}})
--    end),
...
--    awful.key({ modkey }, "p", function () awful.util.spawn_with_shell("dmenu_run -h '15' -i -nb '" .. beautiful.bg_normal .. "' -nf '" .. beautiful.fg_normal .. "' -sb '" .. beautiful.bg_focus .. "' -sf '" .. beautiful.fg_focus .. "' -fn 'DejaVu Sans:size=8'") end),
--      awful.key({ modkey }, "p", function () awful.util.spawn_with_shell("dmenu_run -p 'choose one...' -i -fn 'Terminus:size=9' -nb '#090909' -nf '#969696' -sb '#334d24' -sf '#090909'") end),

--   awful.key({ modkey }, "p", function () awful.util.spawn_with_shell("dmenu_run -i -nb '" .. beautiful.bg_normal .. "' -nf '" .. beautiful.fg_normal .. "' -sb '" .. beautiful.bg_focus .. "' -sf '" .. beautiful.fg_focus .. "' -fn 'Terminus:size=9'") end),
    ...
    -- Screenshot
--awful.key({ }, "Print", function () awful.util.spawn_with_shell("scrot -e 'mv $f $HOME/Pictures/Screenshots'") end),
--awful.key({ modkey, "Shift" }, "Print", function () awful.util.spawn_with_shell("scrot -s") end),

    -- Applications
    awful.key({ modkey }, "d", function () awful.util.spawn_with_shell("firefox") end),
    awful.key({ modkey }, "a", function () awful.util.spawn_with_shell("renoise") end),
    awful.key({ modkey }, "t", function () awful.util.spawn_with_shell("spacefm") end),
    awful.key({ modkey, "Shift" }, "t", function () awful.util.spawn_with_shell("gksu spacefm") end),
--    awful.key({ modkey }, "c", function () awful.util.spawn_with_shell("calibre") end),
    awful.key({ modkey, "Control"  }, "/", function () awful.util.spawn_with_shell("ebook-viewer") end),
    awful.key({ modkey, "Control"  }, "u", function () awful.util.spawn_with_shell("uget-gtk") end),
    awful.key({ modkey }, "q", function () awful.util.spawn_with_shell("renoise") end)
)
...
awful.rules.rules = {
    ...
    { rule_any = { class = {"mpv", "Sxiv", "Oblogout", "Galculator", "Qbittorrent","Uget-gtk",}, name = {"Event Tester"} },  properties = { floating = true } },
--Set Firefox to always map on tags number 2 of screen 1.
    { rule = { class = "Firefox" },  properties = {tag = tags[1][2]}},
    --Set Pidgin to always map on tags number 3 of screen 1.
    { rule = { class = "Calibre" },  properties = {tag = tags[1][5]}},
    { rule = { class = "ebook-viewer" },  properties = {tag = tags[1][5]}},
    { rule = { class = "Qtractor" },  properties = {tag = tags[1][4]}},
    { rule = { class = "Ardour" },  properties = {tag = tags[1][4]}},
    { rule = { class = "Hydrogen" },  properties = {tag = tags[1][5]}},
}
-- }}}
